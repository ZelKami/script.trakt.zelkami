    ...
    def transitionCheck(self, isSeek=False):
        ...
        if self.isPlaying:
            ...
            if self.isPVR:
                ...
                self.videoDuration = int(utilities._to_sec(
                ...
                if 'id' in self.curVideo and self.isMultiPartEpisode:
                    ...
                    if self.curMPEpisode != epIndex:
                        ...
                        if response is not None:
                            ...
                            self.curVideoInfo = kodiUtilities.kodiRpcToTraktMediaObject('episode', kodiUtilities.getEpisodeDetailsFromKodi(
                                self.curVideo['multi_episode_data'][self.curMPEpisode], ['showtitle', 'season', 'episode', 'tvshowid', 'uniqueid', 'file', 'playcount']))
                        ...
                        if curVideo != self.curVideo:
                            ...
                            if response is not None:
                                ...
                                if utilities.isMovie(self.curVideo['type']):
                                    ...
                                    logger.debug(
                                        "Scrobble Movie type, curVideoInfo: %s" % self.curVideoInfo)
                                ...
                                logger.debug(
                                    "Scrobble Episode type, curVideoInfo: %s" % self.curVideoInfo)
                                logger.debug(
                                    "Scrobble Episode type, traktShowSummary: %s" % self.traktShowSummary)
                                response = self.__scrobble('start')
        ...
        if 'type' in self.curVideo:
            ...
            try:
                self.isPVR = xbmc.getCondVisibility(
                    'Pvr.IsPlayingTv') | xbmc.Player().getPlayingFile().startswith('pvr://')
            ...
            if kodiUtilities.getSettingAsBool('scrobble_movie') or kodiUtilities.getSettingAsBool('scrobble_episode'):
                ...
                result = {'movie': self.traktapi.getMovieSummary(
                    best_id).to_dict()}
    ...
    def __preFetchUserRatings(self, result):
        if result:
            if utilities.isMovie(self.curVideo['type']) and kodiUtilities.getSettingAsBool('rate_movie'):
                ...
                self.curVideoInfo['user'] = {'ratings': self.traktapi.getMovieRatingForUser(

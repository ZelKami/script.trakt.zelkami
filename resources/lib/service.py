...
class traktService:
    ...
    def doManualRating(self, data):
        ...
        if not utilities.isValidMediaType(media_type):
            logger.debug("doManualRating(): Invalid media type '%s' passed for manual %s." % (
                media_type, action))
            return
        ...
        if utilities.isMovie(media_type):
            ...
            if summaryInfo:
                if not summaryInfo['watched']:
                    s = utilities.getFormattedItemName(media_type, summaryInfo)
                    logger.debug(
                        "doMarkWatched(): '%s' is not watched on Trakt, marking it as watched." % s)
                    params = {'movies': [summaryInfo]}
            ...
            logger.debug("doMarkWatched(): '%s - Season %d' has %d episode(s) that are going to be marked as watched." %
                         (data['id'], data['season'], len(summaryInfo['shows'][0]['seasons'][0]['episodes'])))
    ...
    def addEpisodesToHistory(self, summaryInfo, s):
        if len(summaryInfo['shows'][0]['seasons'][0]['episodes']) > 0:
            ...
            result = globals.traktapi.addToHistory(summaryInfo)
            if result:
                kodiUtilities.notification(kodiUtilities.getString(
                    32113), kodiUtilities.getString(32115) % (result['added']['episodes'], s))
            else:
                kodiUtilities.notification(kodiUtilities.getString(32114), s)
...
class syncThread(threading.Thread):
    ...
    def run(self):
        sync = Sync(show_progress=self._isManual, run_silent=self._runSilent,
                    library=self._library, api=globals.traktapi)
        sync.sync()
...
class traktMonitor(xbmc.Monitor):
    ...
    def onScanFinished(self, database):
        if database == 'video':
            self.scanning_video = False
            logger.debug(
                "[traktMonitor] onScanFinished(database: %s)" % database)
...
class traktPlayer(xbmc.Player):
    ...
    def onPlayBackEnded(self):
        xbmcgui.Window(10000).clearProperty('script.trakt.zelkami.ids')
        xbmcgui.Window(10000).clearProperty('script.trakt.zelkami.paused')
        if self._playing:
            logger.debug("[traktPlayer] onPlayBackEnded() - %s" %
                         self.isPlayingVideo())
            self._playing = False
            self.plIndex = None
            data = {'action': 'ended'}
    ...
    def onPlayBackStopped(self):
        ...
        xbmcgui.Window(10000).clearProperty('script.trakt.zelkami.paused')
        if self._playing:
            logger.debug("[traktPlayer] onPlayBackStopped() - %s" %
                         self.isPlayingVideo())
            self._playing = False
            self.plIndex = None
            data = {'action': 'stopped'}
    ...
    def onPlayBackPaused(self):
        if self._playing:
            logger.debug("[traktPlayer] onPlayBackPaused() - %s" %
                         self.isPlayingVideo())
            data = {'action': 'paused'}
            self.action(data)
    ...
    def onPlayBackResumed(self):
        if self._playing:
            logger.debug("[traktPlayer] onPlayBackResumed() - %s" %
                         self.isPlayingVideo())
            data = {'action': 'resumed'}
            self.action(data)
    ...
    def onQueueNextItem(self):
        if self._playing:
            logger.debug("[traktPlayer] onQueueNextItem() - %s" %
                         self.isPlayingVideo())

    # called when players speed changes. (eg. user FF/RW)
    def onPlayBackSpeedChanged(self, speed):
        if self._playing:
            logger.debug("[traktPlayer] onPlayBackSpeedChanged(speed: %s) - %s" %
                         (str(speed), self.isPlayingVideo()))

    # called when user seeks to a time
    def onPlayBackSeek(self, time, offset):
        if self._playing:
            logger.debug("[traktPlayer] onPlayBackSeek(time: %s, offset: %s) - %s" %
    ...
    def onPlayBackSeekChapter(self, chapter):
        if self._playing:
            logger.debug("[traktPlayer] onPlayBackSeekChapter(chapter: %s) - %s" %

...
# read settings
__addon__ = xbmcaddon.Addon('script.trakt.zelkami')
...
def checkExclusion(fullpath: str) -> bool:
    ...
    # Script exclusion
    if xbmcgui.Window(10000).getProperty('script.trakt.zelkami.paused') == 'true' and getSettingAsBool('ExcludeScript'):
        logger.debug(
...
def getInfoLabelDetails(result):
    ...
    if 'id' not in result['item'] or type == 'channel':
        ...
        video_ids = xbmcgui.Window(10000).getProperty('script.trakt.zelkami.ids')
        if video_ids:
